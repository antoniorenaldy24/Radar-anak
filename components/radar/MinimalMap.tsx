...
// Coordinates mapping per RW in Dukuh Sutorejo, Kec. Mulyorejo, Surabaya (RW 01 - RW 09)
const RW_COORDS_MAP: Record<string, [number, number]> = {
  "RW 01": [-7.2600, 112.7845],
  "RW 02": [-7.2605, 112.7855],
  "RW 03": [-7.2610, 112.7865],
  "RW 04": [-7.2635, 112.7890],
  "RW 05": [-7.2640, 112.7895],
  "RW 06": [-7.2645, 112.7905],
  "RW 07": [-7.2650, 112.7915],
  "RW 08": [-7.2620, 112.7875],
...
export function MinimalMap() {
  ...
  const [subjectList, setSubjectList] = useState<SubjectProfile[]>([]);
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        if (verifiedCards.length > 0) {
          ...
          // Zero verified cases in DB - clear map completely (No mockup data!)
          setSubjectList([]);
        ...
        <div className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            ...
            <h2 className="mt-2 max-w-xl font-display text-3xl font-extrabold tracking-tight md:text-4xl text-ink">
              Dukuh Sutorejo, Kec. Mulyorejo, Surabaya (RW 01 - RW 09).
            </h2>
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="relative flex flex-col justify-between h-full border border-ink bg-white p-6 sm:p-8 shadow-[8px_8px_0_0_#121212] transition-all duration-300">
              {selectedSubject ? (
                ...
                <div className="flex flex-col items-center justify-center text-center my-auto p-8 space-y-4 font-mono">
                  <ShieldCheck className="size-12 text-ink/30" />
                  <div>
                    <div className="font-bold text-sm text-ink uppercase">BELUM ADA KASUS TERVERIFIKASI</div>
                    <p className="text-xs text-ink/60 mt-1 font-sans">
                      Database saat ini kosong dari kasus terverifikasi. Seluruh kasus yang diverifikasi valid oleh operator di Dashboard Kanban akan tampil secara otomatis di peta ini.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="mt-6 border-t border-ledger pt-3 flex items-center justify-between font-mono text-[9px] uppercase tracking-widest text-ink/40">
                <span>STATUS: {subjectList.length > 0 ? "VERIFIED LAPANGAN" : "DATABASE BERSIH"}</span>
